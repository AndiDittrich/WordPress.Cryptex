...
class Plugin{
    ...
    protected $_activationRedirectPage = 'admin.php?page=cryptex-about';
    ...
    public function _wp_init(){
        ...
        if (is_admin()){
            ...
            if (get_option('cryptex-activation-redirect', '') === 'about-page'){
                // remove redirect flag - do it twice to avoid infinite redirect issues with broken caching plugins!
                $deleted = update_option('cryptex-activation-redirect', '') && delete_option('cryptex-activation-redirect');
            ...
            // plugin upgraded ?
            if (get_option('cryptex-upgrade', '-') === 'true'){
    ...
    public function setupBackend(){
        ...
        $renderTemplate = function($menuItem){
            ...
            include(CRYPTEX_PLUGIN_PATH.'/views/admin/'.$menuItem['template'].'.phtml');
        };
    ...
    public function addPluginMetaLinks($links, $file){
        ...
        if ($file == 'cryptex/Cryptex.php'){
            $links[] = '<a target="_blank" href="https://twitter.com/andidittrich">'.__('News & Updates', 'cryptex').'</a>';
        }
    ...
    public function addPluginPageSettingsLink($links, $file){
        // current plugin ?
        if ($file == 'cryptex/Cryptex.php'){
            $links[] = '<a href="'.admin_url('admin.php?page=Cryptex'). '">'.__('Settings', 'cryptex').'</a>';
        }
        return $links;
    }
    ...
    public function _wp_plugin_activate(){
        // set activation flag
        add_option('cryptex-activation-redirect', 'about-page');
    }
    ...
    public function showUpgradeMessage(){
        ...
        echo '<strong>Cryptex Plugin Upgrade:</strong> The Plugin has been upgraded to <strong>7.1</strong>';
        echo '</p></div>';
    }
    ...
    public static function run($pluginName){
        ...
        if (self::$__instance==null){
            ...
            if ($version == '0.0.0'){
                // store new version
                update_option('cryptex-version', '7.1');

            // plugin upgraded ?
            }else if (version_compare('7.1', $version, '>')){
                // run upgrade hook
